...
def mmseqs2_align_sensitive(query_seqs, ref_seqs, temp_folder='temp'):
    ...
    try:
        ...
        # --max-seqs 10000 to ensure we get all hits
        ...
        if 'qaln' not in alignment_df.columns:
            # Try to load as custom format with alignment sequences
        ...
        if 'qaln' in alignment_df.columns:
            print("Using MMseqs2 alignment sequences directly")
...
def main():
    ...
    print("shape", grn_proc.data.shape)
    ...
    alignment_df = alignment_df[alignment_df['sequence_identity'] > .15]  # Match mo_small_sensitive.py
    ...
    if failed_ids:
        ...
        print("\nAll sequences found alignments with > 15% identity!")
    
    # Initialize aligner with custom parameters matching mo_small_sensitive.py
    aligner = init_aligner(match_score=2, mismatch_score=-1, 
                         extend_gap_score=-.05, open_gap_score=-15)
    ...
    for batch_start in range(0, len(queries), batch_size):
        ...
        for i in range(batch_start, batch_end):
            ...
            # Check if we have MMseqs2 alignment sequences
            if 'qaln' in best_alignment and 'taln' in best_alignment:
                # Use MMseqs2 alignment directly
                query_aln = best_alignment['qaln']
                target_aln = best_alignment['taln']
                
                # Create formatted alignment for compatibility
                formatted = {
                    'seq1': query_aln,
                    'seq2': target_aln,
                    'midline': '',  # Not needed for our purposes
                    'score': best_alignment['bit_score'],
                ...
                ref_seq_pos = int(best_alignment['target_start']) - 1  # Current position in reference sequence
            ...
            # Only expand if we used Biopython alignment (formatted dict has expected keys)
            if 'qaln' not in best_alignment or 'taln' not in best_alignment:
                try:
                ...
                # For MMseqs2 alignments, use the direct annotation
                rows[q] = new_row
