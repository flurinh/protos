...
def run_default_structure_check() -> None:
    ...
    assert list(yaml_data.keys()) == ["version", "sequences"]
...
def run_override_check() -> None:
    ...
    assert list(yaml_data.keys()) == ["version", "sequences", "constraints", "properties"]
    assert yaml_data["version"] == 1
...
def run_reference_template_check() -> None:
    ...
    assert generated_text.strip().startswith("version:")
