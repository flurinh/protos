...
def verify_key_positions(struct_proc, pdb_id, chain_id):
    ...
    # Key microbial opsin positions (different from GPCRs!)
    key_positions = {
        '3.50': 'D/E (Asp/Glu) - Counterion/Schiff base counterion',
        '7.43': 'K (Lys) - Retinal binding lysine',
        '7.46': 'Retinal binding pocket',
        '3.46': 'Proton acceptor region',
        '2.50': 'Conserved position',
        '6.50': 'Conserved position',
        '1.50': 'Conserved position (if present)',
        '4.50': 'Conserved position',
        '5.50': 'Conserved position'
    }
    ...
    for helix_num in range(1, 8):
        ...
        if not helix_data.empty:
            ...
            for i, (_, res) in enumerate(sorted_data.iterrows()):
                ...
                # Highlight functionally important positions for microbial opsins
                if res['grn'] == '7.43':  # Retinal binding lysine
                    colors.append('gold')
                elif res['grn'] == '3.50':  # Counterion
                    colors.append('red')
                elif res['grn'].endswith('.50'):  # Other conserved x.50 positions
...
def create_snake_plot(struct_proc, pdb_id, chain_id):
    ...
    for helix_num in range(1, 8):
        ...
        if not helix_data.empty:
            ...
            for i, (_, res) in enumerate(helix_data.iterrows()):
                ...
                # Color based on functional importance for microbial opsins
                if res['grn'] == '7.43':  # Retinal binding lysine
                    color = 'gold'
                    size = 20
                elif res['grn'] == '3.50':  # Counterion
    ...
    fig.add_annotation(
        x=700, y=100,
        text="Key: Gold=K(7.43) Retinal, Red=D/E(3.50) Counterion, Orange=x.50 conserved",
        showarrow=False,
        font=dict(size=12)
    )
    ...
    if not retinal_k.empty:
        fig.add_trace(
            go.Scatter3d(
                ...
                text=[f"RETINAL BINDING LYSINE<br>{row['res_name3l']}{row['auth_seq_id']}<br>GRN: 7.43" 
                      for _, row in retinal_k.iterrows()],
                hovertemplate='%{text}<extra></extra>',
                name='Retinal Binding K (7.43)'
            )
        )
    ...
    if not counterion.empty:
        fig.add_trace(
            go.Scatter3d(
                ...
                text=[f"SCHIFF BASE COUNTERION<br>{row['res_name3l']}{row['auth_seq_id']}<br>GRN: 3.50" 
                      for _, row in counterion.iterrows()],
                hovertemplate='%{text}<extra></extra>',
                name='Counterion (3.50)'
            )
        )
...
def create_interactive_3d_with_slider(struct_proc, pdb_id, chain_id):
    ...
    for grn_pos in unique_grns:
        ...
        if not grn_ca.empty:
            ...
            for _, res in grn_ca.iterrows():
                special_label = ""
                if grn_pos == '7.43':
                    special_label = " (RETINAL BINDING LYSINE)"
                elif grn_pos == '3.50':
                    special_label = " (SCHIFF BASE COUNTERION)"
    ...
    fig.add_annotation(
        text="Use slider to navigate GRN positions<br>Gold=Retinal K(7.43), Red=Counterion(3.50), Orange=x.50 conserved",
...
def create_grn_heatmap(struct_proc, pdb_id, chain_id):
    ...
    fig.update_layout(
        title="Microbial Opsin GRN Position Properties (with expand_annotation)<br>*RET* = Retinal binding, *CI* = Counterion, * = x.50 conserved",
