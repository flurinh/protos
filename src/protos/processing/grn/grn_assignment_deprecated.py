...
from processing.grn_utils import *
...
def calculate_missing_ctail_grns(aligned_grns, missing_gene_numbers, query_gene_len, grns_float):
    # Check if we have any aligned GRNs, if not, return empty lists
    if not aligned_grns:
        return [], None
...
def sort_grn_rn_pairs(sorted_grns, zippy):
    # Create an empty list for the sorted pairs
    sorted_pairs = []
    # Loop through the values in A
    for value in sorted_grns:
        # Find the first pair in C that contains the current value in A
        pair = [p for p in zippy if p[0] == value][0]
        # Add the pair to the sorted list
        sorted_pairs.append(pair)
        # Remove the pair from C
