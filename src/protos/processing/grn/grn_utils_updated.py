...
def parse_grn_str2float(grn: str = '1x13'):
    ...
    try:
        ...
        # TM BUNDLE with 'x' notation (e.g., '3x50')
        elif 'x' in grn:
            ...
            if len(region) == 2:
                ...
                tm = int(region)
                rn = float('.' + rn)
        # TM BUNDLE with dot notation (e.g., '3.50')
        elif '.' in grn:
            tm_str, rn_str = grn.split('.')
            tm = int(tm_str)
            rn = float('0.' + rn_str)
        else:
...
def check_str_grn_valid(grn: str = '1x13'):
    # Add support for the dot notation format
    if '.' in grn and 'n.' not in grn and 'c.' not in grn:
        try:
            tm_str, rn_str = grn.split('.')
            tm = int(tm_str)
            rn = int(rn_str)
            # Valid TM values are 1-8
            if 1 <= tm <= 8:
                return True
            return False
        except ValueError:
...
def sort_grns_str(grns: list[str]):
    ...
    """
    # Remove duplicates by using a dictionary to track float values
    # This also normalizes between dot and x notation
    float_dict = {}
    for grn in grns:
        float_val = parse_grn_str2float(grn)
        # Skip invalid GRNs
        if float_val == 0.0 and not (grn == '0x0' or grn == '0.0'):
            continue
        # Store with float value as key to remove duplicates
        float_dict[float_val] = grn
    
    # Convert to list of floats and sort
    grns_float = list(float_dict.keys())
    sorted_grns = sort_grns(grns_float)
    
    # Convert back to standardized string format
...
class GRNConfigManager:
    def __init__(self, path='data/grn/configs/', config_path='config.json', protein_family='gpcr_a'):
