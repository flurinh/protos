...
def mmseqs2_align(query_seq, seqs, temp_folder=None):
    """
    ...
    temp_base = temp_folder or paths.get_subdir_path('sequence', 'alignment_dir')
    temp_mmseqs = os.path.join(temp_base, "mmseqs_tmp")
    os.makedirs(temp_mmseqs, exist_ok=True)
    ...
    write_fasta_file(seqs, os.path.join(temp_base, 'sequences.fasta'))

    with open(os.path.join(temp_base, 'query.fasta'), 'w') as query_file:
    ...
    try:
        ...
        if not use_wsl:
            sequences_fasta = windows_to_wsl_path(os.path.join(temp_base, 'sequences.fasta'))
            query_fasta = windows_to_wsl_path(os.path.join(temp_folder, 'query.fasta'))
            sequences_db = windows_to_wsl_path(os.path.join(temp_mmseqs, 'sequences_db'))
            query_db = windows_to_wsl_path(os.path.join(temp_mmseqs, 'query_db'))
            results_db = windows_to_wsl_path(os.path.join(temp_mmseqs, 'results'))
            tmp_dir = windows_to_wsl_path(temp_mmseqs)
            alignment_tsv = windows_to_wsl_path(os.path.join(temp_base, 'alignment_results.tsv'))
        else:
            sequences_fasta = f"{temp_folder}/sequences.fasta"
            query_fasta = f"{temp_folder}/query.fasta"
            sequences_db = f"{temp_mmseqs}/sequences_db"
            query_db = f"{temp_mmseqs}/query_db"
            results_db = f"{temp_mmseqs}/results"
            tmp_dir = f"{temp_mmseqs}"
            alignment_tsv = f"{temp_base}/alignment_results.tsv"
        ...
        if use_wsl:
            ...
            import shutil
            shutil.rmtree(temp_mmseqs, ignore_errors=True)

        # Load the first round alignment results into a dataframe
        alignment_df = load_alignment_file(os.path.join(temp_base, 'alignment_results.tsv'))
        os.remove(os.path.join(temp_base, 'alignment_results.tsv'))
