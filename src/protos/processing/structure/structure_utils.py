...
This module provides refactored versions of structure analysis functions
...
from protos.io.schemas.schema_definitions import (
...
from protos.io.schemas.interface_definitions import StructureInterface

# Configure logger
logger = logging.getLogger(__name__)

# Import amino acid dictionary
from protos.processing.aa_dicts import AA_DICT
...
def three_to_one_letter_code(three_letter_code: str) -> str:
    ...
    Args:
        three_letter_code: Three-letter amino acid code (e.g., 'ALA')
    ...
    # Search through AA_DICT to find matching three-letter code
    for one_letter, (three_letter, _) in AA_DICT.items():
        if three_letter.upper() == three_letter_code:
            return one_letter
...
def load_structure(file_path: Union[str, Path], structure_id: Optional[str] = None) -> pd.DataFrame:
    ...
    # If the file doesn't exist, it might be a structure ID
    if not file_path.exists() and not os.path.isabs(file_path):
        # Try to interpret as a structure ID and get the path
        struct_id = str(file_path).split('.')[0]  # Strip extension if any
        file_path = get_structure_path(struct_id, create_if_missing=False)
        
        # Use as structure_id if not provided
