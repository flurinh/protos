...
class StructureProcessor(BaseProcessor):
    ...
    def download_structure(self, pdb_id: str, save_to_cache: bool = True) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # Parse the structure
            # load_structure_util expects: pdb_id, folder, structure_id
            # For temp file, use the directory and stem
            tmp_dir = os.path.dirname(tmp_path)
            tmp_stem = Path(tmp_path).stem
            df = load_structure_util(tmp_stem, folder=tmp_dir)
    ...
    def get_entity_id_for_pdb(self, pdb_id: str) -> Optional[str]:
        """Get the entity ID for a PDB ID (deprecated - use entity registry)."""
        # This method is deprecated - entity registry handles IDs internally
        # Return the human-readable name for backward compatibility
        import warnings
        warnings.warn(
            "get_entity_id_for_pdb is deprecated. Entity IDs are managed internally by EntityRegistry.",
            DeprecationWarning,
            stacklevel=2
        )
        return pdb_id
    ...
    def save_structure_as_entity(self, structure_df: pd.DataFrame, pdb_id: str, 
        ...
        Returns:
            Human-readable name of the saved structure
        """
        ...
        # Return human-readable name, not hash ID
        return pdb_id
