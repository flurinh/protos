...
# GRN-based intracellular region definition
# ICL loops and cytoplasmic-facing TM helix ends
ICL_GRN_REGIONS = {
    # Intracellular loops
    "ICL1": (12, None),  # 12.xx
    "ICL2": (34, None),  # 34.xx
    "ICL3": (56, None),  # 56.xx (if present)
    "H8": (8, None),     # 8.xx - cytoplasmic helix
    # TM cytoplasmic ends (odd TMs: high positions >=50, even TMs: low positions <=50)
    "TM1_cyto": (1, lambda pos: pos >= 53),  # 1.53+ is cytoplasmic
    "TM2_cyto": (2, lambda pos: pos <= 50),  # 2.50- is cytoplasmic
    "TM3_cyto": (3, lambda pos: pos >= 50),  # 3.50+ is cytoplasmic
    "TM4_cyto": (4, lambda pos: pos <= 45),  # 4.45- is cytoplasmic
    "TM5_cyto": (5, lambda pos: pos >= 65),  # 5.65+ is cytoplasmic
    "TM6_cyto": (6, lambda pos: pos <= 35),  # 6.35- is cytoplasmic
    "TM7_cyto": (7, lambda pos: pos >= 53),  # 7.53+ is cytoplasmic (after NPxxY)
}
...
def is_cytoplasmic_residue(grn: str) -> bool:
    ...
    try:
        ...
        for region_name, (region_helix, condition) in ICL_GRN_REGIONS.items():
            if helix == region_helix:
                if condition is None:
                    return True  # Full loop (ICL1, ICL2, ICL3, H8)
                elif condition(pos):
                    return True  # Cytoplasmic end of TM
        return False
