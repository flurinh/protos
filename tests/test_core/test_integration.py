...
class TestCoreIntegration:
    ...
    def test_cross_format_entity_tracking(self):
        ...
        # Create processors
        struct_proc = TestStructureProcessor()
        seq_proc = TestSequenceProcessor()
    ...
    def test_drag_and_drop_workflow(self):
        ...
        mmcif_dir = struct_proc.get_subdirectory_path('structure_dir')

        dropped_file = Path(mmcif_dir) / "6xyz.cif"
    ...
    def test_no_hardcoded_paths(self, configure_test_paths):
        """Verify that no hardcoded paths are used anywhere."""
        paths = configure_test_paths
        custom_root = Path(paths.data_root) / "custom" / "location"
        custom_root.mkdir(parents=True, exist_ok=True)
        custom_paths = ProtosPaths(data_root=str(custom_root))
        struct_proc = TestStructureProcessor(paths=custom_paths)

        # Save entity
        struct_proc.save_entity("test", "data")

        # Verify file is in custom location
        expected_file = Path(custom_root) / "structure" / "structure" / "test.cif"
        assert expected_file.exists()

        # Verify registry is in custom location
        registry_file = Path(custom_root) / "global_registry.json"
    ...
    def test_dataset_manager_integration(self):
        ...
        entity_registry.register_entity("1ubq", "structure", "structure/structure/1ubq.cif")
        entity_registry.register_entity("3sn6", "structure", "structure/structure/3sn6.cif")
        ...
        dataset_file = paths.get_dataset_path("structure", "test_dataset")
