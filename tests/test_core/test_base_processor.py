...
class TestBaseProcessorNew:
    ...
    def test_accepts_protospaths(self, configure_test_paths):
        """Test processor accepts ProtosPaths instance."""
        paths = configure_test_paths
        custom_root = Path(paths.data_root) / "custom"
        custom_root.mkdir(exist_ok=True)
        custom_paths = ProtosPaths(data_root=str(custom_root))
        processor = TestProcessor(paths=custom_paths)

        # Should use provided paths
        assert processor.paths == custom_paths
        assert str(processor.paths.data_root) == str(custom_root)
    ...
    def test_drag_and_drop_workflow(self):
        ...
        data_dir = processor.data_path / "data"
        data_dir.mkdir(parents=True, exist_ok=True)
    ...
    def test_subdirectory_paths(self):
        ...
        processor = StructureTestProcessor()
