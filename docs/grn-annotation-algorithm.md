...
12.003               numeric GPCR loop segment
...
is never converted to a float. Numeric GPCR utilities remain available for
legacy helix/loop arithmetic.
...
INFER_REFERENCE_GRN_ORDER(table):
    ...
    within each segment:
        ...
        treat GPCR insertion 1.411 as 41 + 1/10, between 1.41 and 1.42
...
This order supplies insertion candidates. It does not rewrite the distributed
CSV or change output column order.
...
### 7. Classify and optionally resolve insertions

```text
FINALIZE_INSERTION(event, intrinsic_grn_order, output):
    ...
    if kind is internal_insertion
       ...
       and candidate count exactly equals inserted residue count:
        ...
        leave residues unnumbered
        report candidates and mark "unassigned"
```

The conservative default is not to invent labels. Exact candidate-count
assignment is useful when the table already defines named loop/gap positions
that are absent in the selected reference but present in the query. If one
residue could occupy either of two empty GRNs, both candidates are reported and
neither is guessed.
...
ASSIGN_GRNS_WITH_EXPANSION(queries, table, family_config):
    ...
    if every seed label is numeric GPCR/loop/tail notation
...
EXPAND_NUMERIC_ANNOTATION(seed, query, config):
    ...
    classify remaining residues as N-side loop, gap, or C-side loop
    generate numeric loop labels from neighboring numeric helices
    sort numeric helix/loop/tail GRNs and report still-unassigned residues
```
...
| Query internal insertion | Detected and anchored; optionally assigned only for an exact number of existing empty candidate GRNs |
...
| Ambiguous insertion placement | Candidates reported; no GRN guessed |
...
| Bulge/constriction absent from table | Projection cannot infer a new structural anomaly; GPCRdb-style anomaly logic remains upstream |
